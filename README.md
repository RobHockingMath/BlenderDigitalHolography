...
The size of your hogel images can be adjusted in render_hogels_from_rig.py.  You can also change VISUAL_DEBUG, which is True by default, to False to improve rendering speed.

Load and run the render_hogels_from_rig.py script to start rendering hogel images.  This might take quite some time, depending on scene complexity and the number of hogels.  Rather than measuring progress within Blender, I find it easier to just look at the folder "hogel_renders" that the images are being written to.  Periodically check how many files are in it (there will be one per hogel, so compute ahead of time how many hogels you have).  
