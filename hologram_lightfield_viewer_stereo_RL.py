"""
hologram_lightfield_viewer_stereo_RL.py
...
Important interaction model:
    ...
    - Stereo Right-Left output for cross-eyed viewing.
...
Stereo output:
    The left half of the window shows the RIGHT-eye view.
    The right half of the window shows the LEFT-eye view.
    This is Right-Left order for cross-eyed viewing.
"""
...
# Stereo params.
# Units are the same world units as the Blender scene. You can put these in
# hogel_params.py later if you want; these defaults are safe if absent.
INITIAL_INTER_EYE_DISTANCE = float(getattr(params, "INITIAL_INTER_EYE_DISTANCE", 0.15))
INTER_EYE_DISTANCE_SPEED = float(getattr(params, "INTER_EYE_DISTANCE_SPEED", 0.25))
MIN_INTER_EYE_DISTANCE = float(getattr(params, "MIN_INTER_EYE_DISTANCE", 0.0))
MAX_INTER_EYE_DISTANCE = float(getattr(params, "MAX_INTER_EYE_DISTANCE", 2.0))
...
class OrbitViewerCamera:
    ...
    def handle_keys(self, keys, dt: float) -> None:
        ...
        # Q/E orbit vertically. Up/down arrows are reserved for stereo eye distance.
...
def camera_basis_for_eye(eye: np.ndarray, target: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Return forward/right/up for a pinhole eye camera looking at target.

    We use per-eye toe-in toward the hologram center. This is simple and works
    well for crossed-eye inspection. The stereo pair is for visualization, not
    calibrated optical display output.
    """
    forward = normalize(target - eye)

    # Avoid degeneracy if the view direction is almost vertical.
    up_ref = WORLD_UP
    if abs(float(np.dot(forward, up_ref))) > 0.98:
        up_ref = np.array([0.0, 1.0, 0.0], dtype=np.float32)
...
def set_eye_uniforms(program, eye: np.ndarray, target: np.ndarray, viewport_w: int, viewport_h: int) -> None:
    cam_forward, cam_right, cam_up = camera_basis_for_eye(eye, target)
    ...
    program["u_view_aspect"].value = max(1, viewport_w) / max(1, viewport_h)
...
def print_controls() -> None:
    print("Controls")
    print("  Output           stereo pair in Right-Left order for cross-eyed viewing")
    ...
    print("  Left/Right       orbit viewer left/right")
    print("  Q/E              orbit viewer down/up")
    print("  Up/Down          increase/decrease inter-eye distance")
    print("  Shift            move/orbit/eye-distance faster")
...
def main() -> None:
    ...
    pygame.display.set_caption("Hogel / Light-Field Viewer - stereo Right-Left - tiled full bilinear")
    ...
    inter_eye_distance = max(MIN_INTER_EYE_DISTANCE, min(MAX_INTER_EYE_DISTANCE, INITIAL_INTER_EYE_DISTANCE))
    ...
    while running:
        ...
        eye_speed = INTER_EYE_DISTANCE_SPEED
        if keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]:
            eye_speed *= FAST_MOVE_MULTIPLIER
        if keys[pygame.K_UP]:
            inter_eye_distance = min(MAX_INTER_EYE_DISTANCE, inter_eye_distance + eye_speed * dt)
        if keys[pygame.K_DOWN]:
            inter_eye_distance = max(MIN_INTER_EYE_DISTANCE, inter_eye_distance - eye_speed * dt)

        width, height = pygame.display.get_surface().get_size()
        width = max(2, width)
        height = max(1, height)

        # Central orbit camera. The stereo eyes are offset along its local right axis.
        center_eye = camera.eye()
        center_right = camera.right()

        left_eye = center_eye - 0.5 * inter_eye_distance * center_right
        right_eye = center_eye + 0.5 * inter_eye_distance * center_right

        # Right-Left stereo order for crossed-eye viewing:
        #   left half of window  = RIGHT eye
        #   right half of window = LEFT eye
        left_view_w = width // 2
        right_view_w = width - left_view_w
        ...
        # Left half: RIGHT-eye image.
        ctx.viewport = (0, 0, left_view_w, height)
        set_eye_uniforms(program, right_eye, HOLOGRAM_CENTER, left_view_w, height)
        vao.render(moderngl.TRIANGLES)

        # Right half: LEFT-eye image.
        ctx.viewport = (left_view_w, 0, right_view_w, height)
        set_eye_uniforms(program, left_eye, HOLOGRAM_CENTER, right_view_w, height)
        vao.render(moderngl.TRIANGLES)
        ...
        if now - last_caption_update > 0.25:
            ...
            pygame.display.set_caption(
                f"Hogel Viewer | stereo RL/cross-eye | tiled full bilinear | {fps:5.1f} FPS | "
                f"{grid} | convention 0x{current_convention_index:X} | "
                f"center_eye=({center_eye[0]:.2f}, {center_eye[1]:.2f}, {center_eye[2]:.2f}) | "
                f"radius={camera.radius:.2f} | IED={inter_eye_distance:.3f}"
            )
